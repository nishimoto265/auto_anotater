...
class CoordinateConverter:
    ...
    def screen_to_yolo_delta(self, delta, size, zoom_level):
        """スクリーン座標のデルタをYOLO座標のデルタに変換"""
        dx = delta.x() / (size.width() * zoom_level)
        dy = delta.y() / (size.height() * zoom_level)
        return (dx, dy)
        
    def screen_to_yolo_point(self, point):
        """スクリーン座標の点をYOLO座標の点に変換"""
        x = point[0] / self.image_width
        y = point[1] / self.image_height
        return (x, y)
        
    def screen_to_yolo(self, rect, size, zoom_level, pan_offset):
        """QRectオブジェクトからYOLO形式に変換（PyQt6対応）"""
        if hasattr(rect, 'x'):
            # QRect オブジェクトの場合
            x = (rect.x() - pan_offset.x()) / (size.width() * zoom_level)
            y = (rect.y() - pan_offset.y()) / (size.height() * zoom_level)
            w = rect.width() / (size.width() * zoom_level)
            h = rect.height() / (size.height() * zoom_level)
        else:
            # タプルの場合
            x, y, w, h = rect
            x = (x - pan_offset.x()) / (size.width() * zoom_level)
            y = (y - pan_offset.y()) / (size.height() * zoom_level)
            w = w / (size.width() * zoom_level)
            h = h / (size.height() * zoom_level)
        
        return self._validate_yolo_coordinates(x, y, w, h)