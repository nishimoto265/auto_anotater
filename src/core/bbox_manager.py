...
class BBoxManager:
    def __init__(self, image_width: int = 800, image_height: int = 600):
        ...
        self.coord_converter = CoordinateConverter(image_width, image_height)
    ...
    def move_bbox(self, bbox: BoundingBox, delta: Tuple[float, float]) -> None:
        """バウンディングボックスを移動"""
        if bbox in self.boxes:
            bbox.yolo_coords[0] += delta[0]
            bbox.yolo_coords[1] += delta[1]
            bbox.timestamp = time.time()
    
    def update_temp_bbox(self, coords: Tuple[float, float, float, float]) -> None:
        """一時的なバウンディングボックスを更新（描画用）"""
        # 実装は必要に応じて追加
        pass
        
    def create_bbox(self, coords: Tuple[float, float, float, float]) -> BoundingBox:
        """新しいバウンディングボックスを作成"""
        return self.create_box(coords, 0, 0)  # デフォルト値